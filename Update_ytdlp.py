...
# 🔥 Update M3U blok met exacte kanaalnaam match (zonder emoji)
def update_channel(lines, name, new_url, referer):
    """
    Vind EXTINF regel met kanaalnaam (na komma) en herschrijf alles eronder:
    user-agent + referer + stream URL
    """
    REFERRER = f"#EXTVLCOPT:http-referrer={referer}"

    # kanaalnaam normaliseren: alleen echte naam (laat emoji weg)
    ...
    while i < len(lines):
        ...
        if line.startswith("#EXTINF") and channel_name in line:
            print(f"🎯 EXACT match gevonden: {channel_name}")
    ...
    print(f"❌ Geen match voor kanaal: {channel_name}")
    return False
